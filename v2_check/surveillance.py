...
def on_message(mqttsub, obj, msg):
    ...
    if x['port'] == 4  and x['dev_id'] in device : 
        ...
        for gw in gateways: msg[11] = gw["rssi"]
...
def write(payload,table):
        mycursor = mydb.cursor()
        try:
            mycursor.execute('INSERT INTO {} (id,index1,index2,index3,index4,value1,value2,value3,value4,presence,time,rssi) VALUES ("{}","{}","{}","{}","{}","{}","{}","{}","{}","{}","{}","{}")'.format(table,payload[0],payload[1],payload[2],payload[3],payload[4],payload[5],payload[6],payload[7],payload[8],payload[9],payload[10],payload[11]))
        except Exception as e :
            print(e)
        mydb.commit()
