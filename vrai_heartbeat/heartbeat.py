...
def on_message(mqttc,obj,msg):
    try:
         ...
         if int(x['port']) == 190 :
            bat = x["payload_fields"]["bat"]
            hour = x["payload_fields"]["hour"]
            minute = x['payload_fields']['minute']
            pressure = x['payload_fields']['pressure']
            ratio = x['payload_fields']['ratio']
            temp = x['payload_fields']['temp']
            data_rate = x['metadata']['data_rate']
            channel =""
            rssi=""
            snr=""
            for el in x['metadata']['gateways'] :
                channel = el['channel']
                rssi = el['rssi']
                snr = el['snr']
            print( "bat : " ,  bat)
